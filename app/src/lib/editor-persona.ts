 ...
 * Override with CENTINEL_EDITOR_PERSONA_PATH; defaults to ~/plans/centinel/EDITOR_PERSONA.md.
 *
...
function editorPersonaPath(): string {
  const raw =
    process.env.CENTINEL_EDITOR_PERSONA_PATH ??
    "~/plans/centinel/EDITOR_PERSONA.md";
  if (raw.startsWith("~/")) return path.join(os.homedir(), raw.slice(2));
  return path.resolve(raw);
}

const PERSONA_PREFIX = `You are the Editor for Centinel — head of the investigative unit, chat persona.
...
const FALLBACK_PERSONA = `You are the Editor for Centinel — head of the investigative unit. You read the
