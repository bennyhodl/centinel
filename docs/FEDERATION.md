...
| **Embeddings, index** | **Optional, and no longer portable by construction** | The operator's call. Chunk identity is still `hash(chunk_text)` (SPEC §6.1), so a peer's vectors *key* cleanly. What changed is the rest of the sentence this row used to make: it read *"vectors are portable because the embedding model is **fixed for every install**"*, and §6.2 un-fixed it on 2026-08-03 — the registry now ships a 2,560-dim default beside a 1,024-dim alternate, described in the code as a different vector space. A transferred vector table therefore has to have its model **checked**, not assumed. `vectors.lance/` records its model in schema metadata and refuses a query vector from any other, so the check now exists on one node; **what no longer exists is a reason two nodes agree.** See [#25](https://github.com/bennyhodl/centinel/issues/25). |
