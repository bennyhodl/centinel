...
Design specs for the five new skills required by Tampa-DOGE. **These are NOT in `~/.hermes/skills/`** — they live here in the plan directory as authored specs. When the project graduates from 🧠 Thinking → 🔬 Investigating → 🛠️ Building, each spec gets translated into a real Hermes skill via `skill_manage(action='create')`.
...
- **Wiki path:** `<wiki>` resolves to `~/wiki/<City>/` per operator. Tampa-DOGE = `~/wiki/Tampa/`.
...
- **Frontmatter on everything:** all wiki pages get YAML frontmatter per `llm-wiki` conventions plus Tampa-DOGE-specific fields (sitemap_entry, vault_paths, investigation, watch_hits).
